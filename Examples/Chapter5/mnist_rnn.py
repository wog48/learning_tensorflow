...
# Replace placeholder for input and lables
_inputs = tf.placeholder(tf.float32, shape=[None, time_steps, element_size], name='inputs')
y = tf.placeholder(tf.float32, shape=[None, num_classes], name='labels')

def varaible_summaries(var):
...
with tf.name_scope('rnn_weights'):
    with tf.name_scope('W_x'):
        ...
        varaible_summaries(Wx)
    with tf.name_scope('W_h'):
        Wh = tf.Variable(tf.zeros([hidden_layer_size, hidden_layer_size]))
        varaible_summaries(Wh)
    with tf.name_scope('Bias'):
        b_rnn = tf.Variable(tf.zeros([hidden_layer_size]))
        varaible_summaries(b_rnn)
...
all_hidden_states = tf.scan(fn=rnn_step, elems=processed_input, initializer=inital_hidden, name='states')
...
with tf.Session() as sess:
    ...
    for i in range(10000):
        ...
        if i % 1000 == 0:
            ...
            print("Iter " +str(i) + ", Loss in batch = " + "{:.6f}".format(loss) + ", Accuracy train data = " + "{:.5f}".format(acc))
        if i % 10:
    ...
    print("Accuracy test data: ", test_acc)
